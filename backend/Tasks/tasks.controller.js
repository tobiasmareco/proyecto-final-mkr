...
export const createTaskController = async (req, res) => { // Definimos el controlador createTaskController
    const {
        result,
        error
...
export const deleteTaskController = async (req, res) => {
    ...
    const {
        result,
        error
    } = await deleteTaskService(taskId) // Llama a la función deleteTaskService

    if (error) { // Si hay un error en el servicio, envía una respuesta con el mensaje de error y el resultado obtenido
        return res.status(error.statusCode).json({
            response: error.response,
            msg: error.msg,
            result
